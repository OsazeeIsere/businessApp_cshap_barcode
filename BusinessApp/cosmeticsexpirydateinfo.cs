...
{
	public partial class cosmeticsexpirydateinfo
	{
		internal cosmeticsexpirydateinfo()
		{
			InitializeComponent();
		}
		...
		private void cosmeticsexpirydateinfo_Load(object sender, System.EventArgs e)
		{
			try // Use TimeSpan and some date calculaton, this should work:
			{
           ...
           //     lbtel.Text = dtidentity.Rows[0]["telephone"].ToString();
				...
				dtgetcosmetics = getdatabase("Select cosmeticsid,cosmeticsname,cosmeticsquantity,cosmeticsunitcostprice,cosmeticsunitsalesprice,expirydate,suppliername,supplierphonenumber,datepurchased,amountpaid,invoicenumber,entrydate From cosmeticsexpirydate order by cosmeticsname");
				if (dtgetcosmetics.Rows.Count > 0)
				{
					...
					for (var j = 0; j < dtgetcosmetics.Rows.Count; j++)
					{
						...
						lstitem.SubItems.Add(dtgetcosmetics.Rows[j]["suppliername"].ToString());
						lstitem.SubItems.Add(dtgetcosmetics.Rows[j]["supplierphonenumber"].ToString());
						lstitem.SubItems.Add(dtgetcosmetics.Rows[j]["datepurchased"].ToString());
						lstitem.SubItems.Add(dtgetcosmetics.Rows[j]["amountpaid"].ToString());
						lstitem.SubItems.Add(dtgetcosmetics.Rows[j]["invoicenumber"].ToString());
		...
		private void check_Click(object sender, System.EventArgs e)
		{
			try
			{
				...
				{
					dtgetproduct = getdatabase("Select * from cosmeticsexpirydate");
					if (dtgetproduct.Rows.Count > 0)
					{
						...
						for (var i = 0; i < dtgetproduct.Rows.Count; i++)
						{
							...
							{
								lstitem.Text = dtgetproduct.Rows[i]["cosmeticsid"].ToString();
								lstitem.SubItems.Add(dtgetproduct.Rows[i]["cosmeticsname"].ToString());
								lstitem.SubItems.Add(dtgetproduct.Rows[i]["cosmeticsquantity"].ToString());
								lstitem.SubItems.Add(dtgetproduct.Rows[i]["cosmeticsunitcostprice"].ToString());
								lstitem.SubItems.Add(dtgetproduct.Rows[i]["cosmeticsunitsalesprice"].ToString());
				...
				{
					System.Data.DataTable dtgetcosmetics = new System.Data.DataTable();
					dtgetcosmetics = getdatabase("Select * From cosmeticsexpirydate order by cosmeticsname");
					if (dtgetcosmetics.Rows.Count > 0)
					{
						ListViewItem lstitem = new ListViewItem();
						lsvitems.Items.Clear();
				...
				if (!string.IsNullOrEmpty(txtitems.Text))
				{
					...
					for (var i = 2; i < 2 + intitems; i++)
					{
						...
						if (dtgetproduct.Rows.Count > 0)
						{
							...
							{
								lstitem = new ListViewItem();
								lstitem.Text = dtgetproduct.Rows[j]["cosmeticsid"].ToString();
								lstitem.SubItems.Add(dtgetproduct.Rows[j]["cosmeticsname"].ToString());
								lstitem.SubItems.Add(dtgetproduct.Rows[j]["cosmeticsquantity"].ToString());
								lstitem.SubItems.Add(dtgetproduct.Rows[j]["cosmeticsunitcostprice"].ToString());
								lstitem.SubItems.Add(dtgetproduct.Rows[j]["cosmeticsunitsalesprice"].ToString());
		...
		private void txtsearch_TextChanged(object sender, System.EventArgs e)
		{
			...
			{
				System.Data.DataTable dtgetproduct = new System.Data.DataTable();
				dtgetproduct = getdatabase("Select * From cosmeticsexpirydate Where cosmeticsname Like '%" + txtsearch.Text + "%' Order By cosmeticsname;");
				if (dtgetproduct.Rows.Count > 0)
				{
					ListViewItem lstitem = new ListViewItem();
					lsvitems.Items.Clear();
