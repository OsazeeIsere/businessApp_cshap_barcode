...
{
	public partial class drinkslog
	{
			...
			try
			{
				...
				for (var i = 0; i < dtgetadmin.Rows.Count; i++)
				{
					if (txtpassword1.Text == dtgetadmin.Rows[i]["adminpassword"].ToString() && cbocashier.Text == "All Sales")
					{
						...
						dtgetsaleslog = getdatabase("select * from drinkslog order by date");
						dgvdrinkslog.DataSource = dtgetsaleslog;
						// generate another table for necessary calculation

						dtgetsaleslog1 = getdatabase("select amountsold,profit from drinkslog");
					...
					{
						dtgetsaleslog = getdatabase("select * from drinkslog where cashiername='" + cbocashier.Text + "' And date='" + Convert.ToDateTime(DateTimePicker1.Value.Date) + "'");
						if (dtgetsaleslog.Rows.Count > 0)
						{
							c = dtgetsaleslog.Rows.Count;
						}
						txtcustomer.Text = c.ToString();
						dtgetsaleslog1 = getdatabase("select amountsold,profit from drinkslog where cashiername='" + cbocashier.Text + "' And date='" + Convert.ToDateTime(DateTimePicker1.Value.Date) + "'");
						if (dtgetsaleslog1.Rows.Count > 0)
						{
					...
					{
						dtgetsaleslog = getdatabase("select * from drinkslog where date >='" + Convert.ToDateTime(DateTimePicker1.Value.Date) + "' And date <='" + Convert.ToDateTime(DateTimePicker2.Value.Date) + "' And cashiername='" + cbocashier.Text + "'");
						if (dtgetsaleslog.Rows.Count > 0)
						{
							c = dtgetsaleslog.Rows.Count;
						}
						txtcustomer.Text = c.ToString();
						dgvdrinkslog.DataSource = dtgetsaleslog;
						dtgetsaleslog1 = getdatabase("select amountsold,profit from drinkslog where date >='" + Convert.ToDateTime(DateTimePicker1.Value.Date) + "' and date <='" + Convert.ToDateTime(DateTimePicker2.Value.Date) + "' and cashiername='" + cbocashier.Text + "'");
						if (dtgetsaleslog1.Rows.Count > 0)
						{
							for (var k = 0; k < dtgetsaleslog1.Rows.Count; k++)
							{
					...
					{
						dtgetsaleslog = getdatabase("select * from drinkslog where date='" + Convert.ToDateTime(DateTimePicker1.Value.Date) + "'");
						if (dtgetsaleslog.Rows.Count > 0)
						{
							c = dtgetsaleslog.Rows.Count;
						}
						txtcustomer.Text = c.ToString();
						dtgetsaleslog1 = getdatabase("select amountsold,profit from drinkslog where date='" + Convert.ToDateTime(DateTimePicker1.Value.Date) + "'");
						if (dtgetsaleslog1.Rows.Count > 0)
						{
							for (var j = 0; j < dtgetsaleslog1.Rows.Count; j++)
							{
					...
					{
						dtgetsaleslog = getdatabase("select * from drinkslog where date >='" + Convert.ToDateTime(DateTimePicker1.Value.Date) + "' And date <='" + Convert.ToDateTime(DateTimePicker2.Value.Date) + "'");
						if (dtgetsaleslog.Rows.Count > 0)
						{
							c = dtgetsaleslog.Rows.Count;
						}
						txtcustomer.Text = c.ToString();
						dgvdrinkslog.DataSource = dtgetsaleslog;
						dtgetsaleslog1 = getdatabase("select amountsold,profit from drinkslog where date >='" + Convert.ToDateTime(DateTimePicker1.Value.Date) + "' and date <='" + Convert.ToDateTime(DateTimePicker2.Value.Date) + "'");
						if (dtgetsaleslog1.Rows.Count > 0)
						{
							for (var k = 0; k < dtgetsaleslog1.Rows.Count; k++)
							{
		...
		private void drinkslog_Load(object sender, System.EventArgs e)
		{
			try
			{
				drinkslog x = new drinkslog();
				double totalsales = 0;
				double totalprofit = 0;
				System.Data.DataTable dtgetsaleslog = new System.Data.DataTable();
				dtgetsaleslog = getdatabase("select * from drinkslog order by date");
				...
				{
					for (var k = 0; k < dtgetcashier.Rows.Count; k++)
					{
						cbocashier.Items.Add(dtgetcashier.Rows[k]["cashiername"]).ToString();
					}
				}
